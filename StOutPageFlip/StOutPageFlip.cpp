...
#define log(...) __android_log_print(ANDROID_LOG_INFO, "StOutPageFlip", __VA_ARGS__)
...
//LUK: added
#if defined(__ANDROID__)
static int64_t frameTime_prev = 0;

static float base_advance = 0.5f; //also set as initial value in gui!!!
...
static void updateValues(struct nvstusb_context *ctx, nvstusb_eye cur_eye, int64_t frameTimeNanos)
{
  //LUK: 500 seems to work best now with PLL in the pico emitter !!!
  //     10000 seemsto work best with new fw
  nvstusb_set_delay(ctx, base_advance * 10000);
  
  nvstusb_set_eye(ctx, cur_eye);
}

static float pba = 0.5f;

static void frameCallback_old(long frameTimeNanos, void* data)
{
  StOutPageFlip *inst = (StOutPageFlip *)data;
  
  inst->postFrameCallback_old(inst->choreographer, frameCallback_old, inst);

  if(inst->ctx != NULL) {
    base_advance = inst->getDelay();
    if(pba != base_advance) {
      nvstusb_set_rate(inst->ctx, 120.0f + (0.5f - base_advance)*0.25f);
      pba = base_advance;
    }
    updateValues(inst->ctx, inst->cur_eye, frameTimeNanos);
  }
  
  //inst->myContext->StSetEglPresentationTime(frameTimeNanos + 2*8133333);
  inst->myContext->StSetEglPresentationTime(frameTimeNanos + 3*8333333 - (getNanos()-frameTimeNanos) );
  //inst->myContext->StSetEglPresentationTime(frameTimeNanos + 3*8133333 - (frameTimeNanos-frameTime_prev) );

  inst->stglDrawAggressive(inst->cur_eye+1);
  
  inst->cur_eye = (nvstusb_eye)(!inst->cur_eye);
  
  frameTime_prev = frameTimeNanos;
}
...
static void frameCallback(int64_t frameTimeNanos, void* data)
{
  ...
  inst->postFrameCallback(inst->choreographer, frameCallback, inst);
...
void StOutPageFlip::setupDevice() {
    switch(myDevice) {
	    ...
	    int usb_fd = -1;
#if defined(__ANDROID__)
	    StNativeWin_t p = getParentWindow();
	    usb_fd = p->getUSBFileDescriptor();
	    log(" LUK usb_fd: %d", usb_fd);
#endif	      	    
	    const int dev = 1; //LUK: nvidia or pico	    
	    ctx = nvstusb_init((dev==1) ? "nvstusb.fw" : NULL, usb_fd);
	    if (0 == ctx) {
	      if(dev)
		fprintf(stderr, "Could not initialize NVIDIA 3D Stereo Controller\n");
	      else
		fprintf(stderr, "Could not initialize Pico 3D Stereo Controller\n");
	    }
	    else if(delay > 0) {
	      ...
	      const StSearchMonitors& aMonitors = StWindow::getMonitors();
	      const StMonitor aMon = getHigestFreqMonitor(aMonitors);
	      float freq = aMon.getFreq();
#if defined(__ANDROID__)
	      StNativeWin_t p = getParentWindow();
	      freq = p->getRefreshRate();
#endif	      
	      nvstusb_set_rate(ctx, freq);
	    }
	    
	    //LUK: added
#if defined(__ANDROID__)	    
	    void *hlib = dlopen("libandroid.so", 0);
	    assert(hlib != NULL);
	    
	    AChoreographer_getInstance func = (AChoreographer_getInstance)dlsym(hlib, "AChoreographer_getInstance");
	    assert(func != NULL);
	    
	    choreographer = func();
	    assert(choreographer != NULL);

	    postFrameCallback = (AChoreographer_postFrameCallback64)dlsym(hlib, "AChoreographer_postFrameCallback64");

	    //LUK: try for dlp projector
	    if(postFrameCallback != NULL) {
	      assert(postFrameCallback != NULL);

	      setDelay(base_advance);   
	      cur_eye = (nvstusb_eye)(ST_DRAW_LEFT&1);
 
	      postFrameCallback(choreographer, frameCallback, this);
	    }
	    else {
	      
	      postFrameCallback_old = (AChoreographer_postFrameCallback)dlsym(hlib, "AChoreographer_postFrameCallback");
	      assert(postFrameCallback_old != NULL);

	      setDelay(base_advance);   
	      cur_eye = (nvstusb_eye)(ST_DRAW_LEFT&1);

	      postFrameCallback_old(choreographer, frameCallback_old, this);	              
	    }

#if 0	    
	    StNativeWin_t p = getParentWindow();
	    ANativeWindow *nw = p->getWindow(); //p is StAndroidGlue* type

	    void *hlib = dlopen("libnativewindow.so", 0);
	    assert(hlib != NULL);

	    typedef int32_t (*ANativeWindow_setFrameRate)(ANativeWindow *window,
							  float frameRate,
							  int8_t compatibility);
	    
	    ANativeWindow_setFrameRate setRate_callback =
	      (ANativeWindow_setFrameRate)dlsym(hlib, "ANativeWindow_setFrameRate");
	    assert(setRate_callback != NULL);
	    
	    setRate_callback(nw, 120.0f, 0);
#endif
...
void StOutPageFlip::processEvents() {
    ...
    if(aKeys.isKeyDown( ST_VK_S )) {
      delay += 10;
      printf("nvstusb: eye swap delay: %d\n", delay);
      if(ctx != 0)
	nvstusb_set_delay(ctx, delay);
      aKeys.keyUp(ST_VK_S, aKeys.getKeyTime(ST_VK_S));
      return;
    }
    if(aKeys.isKeyDown( ST_VK_W )) {
      printf("nvstusb: swap eyes\n");
      if(ctx != 0)
	nvstusb_invert_eyes(ctx);
      aKeys.keyUp(ST_VK_W, aKeys.getKeyTime(ST_VK_W));
      return;
    }   
}
...
void StOutPageFlip::stglDrawAggressive(unsigned int theView) {
    ...
    /*
      //LUK:
      sView uses glXSwapIntervalEXT for vsync (always on for quad buffer emulation);
      this corresponds to vblank_method = 3, meaning that eye swapping has to be done 
      after swapping GL buffers;
    */
#if !defined(__ANDROID__)
    //LUK: added for linux on asus!!!    
    glFinish();
#endif    
    StWindow::stglSwap(ST_WIN_MASTER);
    ++myFPSControl;

    //LUK: swap the eyes; for android done in frameCallback
#if !defined(__ANDROID__)
    if(ctx != 0) {
      //glFinish(); //LUK: inverts eyes (next frame)!!!
      //usleep(delay);
      //nvstusb_set_delay(ctx, delay);      
      nvstusb_set_eye(ctx, (nvstusb_eye)(theView&1));
    }
#endif    
