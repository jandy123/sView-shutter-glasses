...
void StOutPageFlipExt::doSetDeviceControl(const int32_t theValue) {
    switch(theValue) {
        ...
        case DEVICE_CONTROL_WHITELINE:
            ...
            getDeviceControl()->setMode(StGLDeviceControl::OUT_UNDEFINED);
            break;
        case DEVICE_CONTROL_TRIGGER_BOX:
            getDeviceControl()->setMode(StGLDeviceControl::OUT_UNDEFINED);
            break;
...
void StOutPageFlipExt::updateStringsExt() {
    ...
    params.ControlCode->defineOption(DEVICE_CONTROL_TRIGGER_BOX, myLangMap.changeValueId(STTR_PARAMETER_CONTROL_TRIGGERBOX, "Trigger box sync"));
...
: StOutPageFlip(theResMgr, theParentWindow),
  ...
  myIsQuiting(false) {
    ...
    params.ControlCode->defineOption(DEVICE_CONTROL_TRIGGER_BOX, stCString("triggerBox"));
...
void StOutPageFlipExt::releaseResources() {
    if(!myContext.isNull()) {
        ...
        myTriggerBox.release(*myContext);
...
bool StOutPageFlipExt::create() {
    ...
    myTriggerBox.stglInit(*myContext);
