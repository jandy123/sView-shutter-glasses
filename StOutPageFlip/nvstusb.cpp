...
struct nvstusb_context {
  ...
  enum nvstusb_dev_type dev_type; //LUK: device type 1: nvidia, 0: pico

  int extra_delay; //LUK: added for pico
  long extra_time; //LUK: added for pico
};

/* initialize controller */
struct nvstusb_context *
nvstusb_init(char const * fw, int fileDescriptor) 
{
  ...
  /* open device */
  struct nvstusb_usb_device *dev = (nvstusb_usb_device *)nvstusb_usb_open_device(fw? fw : NULL);
  if (0 == dev) return 0;

#else
 
  /* open device */
  struct nvstusb_usb_device *dev = (nvstusb_usb_device *)nvstusb_usb_open_device_jni(fileDescriptor);
  if (0 == dev) return 0;
 
#endif  
  
  /* allocate context */
  struct nvstusb_context *ctx = (nvstusb_context *)malloc(sizeof(*ctx));
  if (0 == ctx) {
  ...
  ctx->rate = 0.0;
  ctx->eye = (nvstusb_eye)0;
  ctx->device = dev;
  ctx->vblank_method = 0;
  ctx->toggled3D = 0;
  ctx->invert_eyes = 0;
  ctx->b_thread_running = 0;
  ctx->dev_type = (fw != NULL) ? nvstusb_nvidia : nvstusb_pico;
  ctx->extra_delay = 0;
  ctx->extra_time = 0;
  ...
  //LUK: override!!!
  ctx->vblank_method = 3;
  fprintf(stderr, "nvstusb: FORCED vblank method: %d\n", ctx->vblank_method);
#else  
...
nvstusb_set_rate(
  ...
  if(ctx->dev_type == nvstusb_pico) {

    uint8_t buf[8] = { 
      0x30, 0x2a, 0x35, 0x00, 0x00, 0x00, 0x00, 0x00
    };
    snprintf((char*)buf+3, 5, "%d", (int)ctx->rate);
  ...
  /* some timing voodoo */
  rate = 99.7f; //LUK: !!!!
  int32_t frameTime   = (1000000.0/rate);
  int32_t activeTime  = 2080;                 /* 2.08000 ms time each eye is on*/

  int32_t w = NVSTUSB_T2_COUNT(0.0f); //LUK: doesn't matter
  int32_t x = NVSTUSB_T0_COUNT(6400.0f);
  int32_t y = NVSTUSB_T0_COUNT(activeTime);
  int32_t z = NVSTUSB_T2_COUNT(frameTime);    
...
nvstusb_set_eye(
  ...
  //LUK: no clue what this, but seems better than orig
  if(ctx->dev_type == nvstusb_nvidia)
    r = 0; //LUK:???
    //r = NVSTUSB_T2_COUNT((1e6/ctx->rate)/1.8);
    //r = NVSTUSB_T2_COUNT((ctx->extra_delay)/1.8);
  ...
  case nvstusb_left:
	...
	uint8_t buf[8] = { 
	  ...
	  //0x00, 0x00,               /* unused */
	  ctx->extra_delay, ctx->extra_delay >> 8, //LUK: abuse these two 0's!!!
	  r, r>>8, r>>16, r>>24     //LUK: no clue if this works!!!
	};
	nvstusb_usb_write_bulk(ctx->device, 1, buf, 8);
      }
      else {
	uint8_t buf[8] = { 
	  ((eye==nvstusb_right)^(ctx->invert_eyes))?0x31:0x32,        /* eye selection */
	  0x3a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
	};
	snprintf((char*)buf+2, 6, "%d", ctx->extra_delay);
	nvstusb_usb_write_bulk(ctx->device, 1, buf, 8);

	nvstusb_usb_read_bulk(ctx->device, 2, buf, 6);
	ctx->extra_time = strtol((char*)buf, NULL, 10); 
      }
    }
    break;
  case nvstusb_quad:
...
nvstusb_get_keys(
  ...
  if(ctx->dev_type == nvstusb_pico) {
    keys->deltaWheel = keys->pressedDeltaWheel = 0;
    keys->toggled3D = 0;
    return;
  }
