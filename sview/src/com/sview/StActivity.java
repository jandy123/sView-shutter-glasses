...
//region Overridden methods of NativeActivity class
	...
	//LUK: added
	cppSetRefreshRate(myCppGlue, getWindowManager().getDefaultDisplay().getRefreshRate());
...
//region Methods to be called from C++ level
    ...
    {
	Log.d(TAG, "LUK: connect device " + device);
        ...
        cppInitializeNativeDevice(myCppGlue, fileDescriptor);

	Log.d(TAG, "LUK: connected device ");
    }
    ...
    protected void checkUsbDevices()
    {
	    ...
	    //LUK: device filter here !!!
	    //if(device.getVendorId() != 0xXXXX || device.getProductId() != 0xXXXX) continue;
...
//region Methods to call C++ code
    ...
    //LUK: added 
    private native void cppSetRefreshRate(long theCppPtr, float rr);
...
//endregion
    ...
    private static final String TAG = "libusb_sview" ;
     ...
     */
    public native void cppInitializeNativeDevice(long theCppPtr, int fileDescriptor);
