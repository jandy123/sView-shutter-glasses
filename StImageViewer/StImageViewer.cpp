...
namespace {
  ...
  //LUK: not doing much
  //static const char ST_SETTING_SWAP_LR[]       = "swapLR";
...
void StImageViewer::updateStrings() {
    ...
    //LUK: added
    params.extraDelay->setName(stCString("Extra delay"));
}
...
: StApplication(theResMgr, theParentWin, theOpenInfo),
  ...
  myToCheckPoorOrient(true) {
    ...
    //LUK: added
    params.extraDelay = new StFloat32Param(0.5f, stCString("extraDelay"));
    params.extraDelay->setMinMaxValues(0.0f, 1.0f);
    params.extraDelay->setDefValue(0.5f);
    params.extraDelay->setStep(0.1f);
    params.extraDelay->setTolerance(0.1f);
    params.extraDelay->setFormat(stCString("%01.1f s"));
    //
    ...
    mySettings->loadParam (params.extraDelay); //LUK:
    ...
    addRenderer(new StOutPageFlipExt(myResMgr, theParentWin)); //LUK: added
#else
    addRenderer(new StOutAnaglyph   (myResMgr, theParentWin));
...
void StImageViewer::saveGuiParams() {
    ...
    //LUK: not doing much
    //mySettings->saveInt32(ST_SETTING_SWAP_LR, myGUI->myImage->params.SwapLR->getValue());
...
void StImageViewer::saveAllParams() {
    ...
    if(!myGUI.isNull()) {
        ...
        mySettings->saveParam (params.extraDelay); //LUK:
...
bool StImageViewer::createGui() {
    ...
    myGUI->delay->setValue(params.extraDelay->getValue()); //LUK: !!!
	...
	//LUK: not doing much
	//int32_t swapLR = 0;	
        //mySettings->loadInt32(ST_SETTING_SWAP_LR, swapLR);
        //myGUI->myImage->params.SwapLR->setValue(swapLR);
...
void StImageViewer::doKeyDown(const StKeyEvent& theEvent) {
	...
	//LUK: deghost?
        case ST_VK_G: {
	  doDeghost = !doDeghost;
	  myLoader->setDeghost(doDeghost);
	  myLoader->doReloadImage();
	  return;
	}	  
        default: break;
    }
