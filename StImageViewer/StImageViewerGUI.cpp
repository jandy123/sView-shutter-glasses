...
void StImageViewerGUI::createImageAdjustments() {
    ...
    //LUK: added
    const float val = myPlugin->getMainWindow()->getDelay();
    delay = new StFloat32Param(val, 0.0f, 1.0f, 0.5f, 0.1f, 0.1f);
    delay->signals.onChanged.connect(this, &StImageViewerGUI::delayChanged);
    myBtnDelay = new StGLRangeFieldFloat32(myAdjustOverlay, delay,
                                                              0, 0, StGLCorner(ST_VCORNER_CENTER, ST_HCORNER_RIGHT),
                                                              StGLRangeFieldFloat32::RangeStyle_Seekbar, scale(18));
    myBtnDelay->changeRectPx().right() = myBtnDelay->getRectPx().left() + aSlideWidth;
    myBtnDelay->changeRectPx().moveTopTo(aCtrlStep * (aBtnIter++));
    myBtnDelay->changeMargins().left   = scale(8);
    myBtnDelay->changeMargins().right  = scale(8);
    myBtnDelay->setCorner(StGLCorner(ST_VCORNER_TOP, ST_HCORNER_LEFT));
    myBtnDelay->setFormat(stCString("Render delay: %+01.1f"));
    ///
...
//LUK: added
void StImageViewerGUI::delayChanged(float val) {
  //StNativeWin_t p = myPlugin->myWinParent; //p is StAndroidGlue* type
  //p->setDelay(val);
  myPlugin->getMainWindow()->setDelay(val);
  myPlugin->params.extraDelay->setValue(val); 
}
