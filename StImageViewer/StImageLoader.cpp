...
static void deghost_1(float gf, StHandle<StImage> anImageL, StHandle<StImage> anImageR)
{
  unsigned int w = anImageL->getSizeX()/2, h = anImageL->getSizeY();
  ...
  //printf("Deghosting image pair: %.2f\n", gf);
  ...
  for(unsigned int j=0; j<h; j++) 
	...
	int xlm = (0.9f+ghosting)*rl - rr*ghosting;
	int xr = (1.0f+ghosting)*rr + rl*ghosting;    
	int xrm = (0.9f+ghosting)*rr - rl*ghosting;    
	if(xl < 0) xl = 0; else if (xl > 254) xl = 255;
	if(xr < 0) xr = 0; else if (xr > 254) xr = 255;
	if(xlm < 0) xlm = 0; else if (xlm > 254) xlm = 255;
	if(xrm < 0) xrm = 0; else if (xrm > 254) xrm = 255;
#if 0	
	//if(xlg - xrg > 5)
	  idata_l[idxl+k] = xl;
...
static void deghost(float gf, StHandle<StImage> anImageL, StHandle<StImage> anImageR)
{
  ...
  if(anImageL->getColorModel() != StImage::ImgColor_RGB) {
    printf("LUK: no RGB color model %d %ld %ld. Skipping deghosting.\n", anImageL->getColorModel(), anImageL->getSizeX(),  anImageL->getSizeY());
    return;
  }
  ...
  for(unsigned int j=0; j<h; j++) 
	...
	idxr = idxl + 3*w;
      ...
      //fac += (2.0f*idxr)/(6*w*h);
      //const float ghosting = gf*fac/(2000.0f+xlg+xrg);
      const float ghosting = gf*fac/2000.0f;
#else  
      int w = ilGetInteger(IL_IMAGE_WIDTH);
      int h = ilGetInteger(IL_IMAGE_HEIGHT);
      float fac = idxr/(3.0f*w*h);
	...
	//int xl = rl + rr*ghosting;
	//int xr = rr + rl*ghosting;
	//int xl = rl + (rl-rr)*2*ghosting;
	//int xr = rr + (rr-rl)*2*ghosting;

	int xl = (1.0f+ghosting)*rl + rr*ghosting;
	int xlm = (1.0f+ghosting)*rl - 2*rr*ghosting;
	int xr = (1.0f+ghosting)*rr + rl*ghosting;    
	int xrm = (1.0f+ghosting)*rr - 2*rl*ghosting;    
	if(xl < 0) xl = 0; else if (xl > 254) xl = 255;
	if(xr < 0) xr = 0; else if (xr > 254) xr = 255;
	if(xlm < 0) xlm = 0; else if (xlm > 254) xlm = 255;
	if(xrm < 0) xrm = 0; else if (xrm > 254) xrm = 255;
...
bool StImageLoader::loadImage(const StHandle<StFileNode>& theSource,
    ...
    //LUK: deghost
    if(doDeghost)
      deghost(1.1f, anImageL, anImageR);
    ...
    for(;;) {
        ...
        switch(myAction) {
            ...
            default: {
                ...
                myLoadNextEvent.reset();
                if(myPlayList->getCurrentFile(aFileToLoad, aFileParams)) {
		  //LUK: overwrite per-file eye inversion
		  if(myImgInfo != 0)
		    aFileParams->ToSwapLR = myImgInfo->Id->ToSwapLR;
