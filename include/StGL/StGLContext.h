...
class StGLContext {
    ...
    typedef EGLBoolean (*eglPresentationTimeANDROID)(
                        EGLDisplay dpy,
                        EGLSurface surface,
                        EGLnsecsANDROID time);
    
    eglPresentationTimeANDROID eglPresentationTime_callback = 0;

 public:
    inline ST_CPPEXPORT void StSetEglPresentationTime(int64_t time) {
      eglPresentationTime_callback(eglGetCurrentDisplay(), eglGetCurrentSurface(EGL_DRAW), (EGLnsecsANDROID)time);
    }
