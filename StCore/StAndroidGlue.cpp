...
void StAndroidGlue::threadEntry() {
    ...
    //LUK: libusb
    int r = libusb_set_option(0, LIBUSB_OPTION_ANDROID_JAVAVM, myJavaVM, 0);
    ST_ERROR_LOG("libusb_set_option ANDROID_JAVAVM: " + libusb_strerror(r));
...
//LUK: added
jexp void JNICALL Java_com_sview_StActivity_cppSetRefreshRate(JNIEnv* theEnv, jobject theObj, jlong theCppPtr, jfloat refreshRate) {

  ((StAndroidGlue* )theCppPtr)->setRefreshRate(refreshRate);
}

//LUK: added

jexp void JNICALL Java_com_sview_StActivity_cppInitializeNativeDevice(JNIEnv* theEnv, jobject theObj, jlong theCppPtr, jint fileDescriptor) {

  ((StAndroidGlue* )theCppPtr)->initializeNativeDevice(fileDescriptor);
}
