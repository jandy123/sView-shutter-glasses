...
LIB_GLX = -lGL -lX11 -lXext -lusb-1.0
...
LIB_OUTPUTS = -lStOutAnaglyph -lStOutInterlace -lStOutDistorted -lStOutPageFlip
endif
...
EXTRA_CFLAGS   += $(ANDROID_SYSROOT) $(ANDROID_MARCH) -Wno-narrowing -Wno-unused-variable -Wno-unused-function -I$(LIBUSB)/include
EXTRA_CXXFLAGS += $(ANDROID_SYSROOT) $(ANDROID_MARCH) -Wno-narrowing -Wno-unused-variable -Wno-unused-function -I$(LIBUSB)/include
...
EXTRA_LDFLAGS  += $(ANDROID_SYSROOT) -L$(ANDROID_NDK)/sources/cxx-stl/gnu-libstdc++/4.9/libs/$(ANDROID_EABI) -lstdc++ -lgnustl_shared -L$(LIBUSB)/libs/$(ANDROID_EABI) -lusb1.0
...
ifeq ($(TARGET_OS),linux)
EXTRA_CXXFLAGS += `pkg-config gtk+-2.0 --cflags` -I/usr/include/libusb-1.0
...
ifdef ANDROID_NDK
outputs_all: $(aStOutAnaglyph) $(aStOutInterlace) $(aStOutDistorted) $(aStOutPageFlip)
else
...
install_android_libs: $(aStShared) $(aStGLWidgets) $(aStCore) $(aStOutAnaglyph) $(aStOutInterlace) $(aStOutDistorted) $(aStOutPageFlip) $(aStImageViewer) $(aStMoviePlayer) $(sViewAndroid)
	...
	cp -f $(BUILD_ROOT)/$(aStOutPageFlip) $(aDestAndroid)/lib/$(ANDROID_EABI)/
	...
	cp -f $(LIBUSB)/libs/$(ANDROID_EABI)/libusb1.0.so       $(aDestAndroid)/lib/$(ANDROID_EABI)/



install_android_cad_libs: $(aStShared) $(aStGLWidgets) $(aStCore) $(aStOutAnaglyph) $(aStOutInterlace) $(aStOutDistorted) $(aStOutPageFlip) $(aStImageViewer) $(aStMoviePlayer) $(sViewAndroidCad)
	cp -f $(BUILD_ROOT)/$(aStShared)       $(aDestAndroid)/lib/$(ANDROID_EABI)/
	cp -f $(BUILD_ROOT)/$(aStGLWidgets)    $(aDestAndroid)/lib/$(ANDROID_EABI)/
	cp -f $(BUILD_ROOT)/$(aStCore)         $(aDestAndroid)/lib/$(ANDROID_EABI)/
	cp -f $(BUILD_ROOT)/$(aStOutAnaglyph)  $(aDestAndroid)/lib/$(ANDROID_EABI)/
